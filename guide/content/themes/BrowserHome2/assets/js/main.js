require(['jquery'], function ($) {
		...
		setLogo: function () {
			...
			if (this.get('logo')) {
				...
				if (this.get('nightMode')) {
					logoUrl = `${THEME.url}/assets/logo/HarmonyOS_logo(for nightMode).png`;
				} else {
					logoUrl = `${THEME.url}/assets/logo/HarmonyOS_logo.png`;
				}
			}
		...
		$.ajax({
			...
			success: (json) => {
				...
				json.unshift(
					{
						"name": "书签",
						"data-url": "openbookmarksList()",
						"icon": `${THEME.url}/assets/img/bookmarks/bookmarks.png`
					},
					{
						"name": "精选",
						"data-url": "choice()",
						"icon": `${THEME.url}/assets/img/bookmarks/discover.png`
					},
					{
						"name": "设置",
						"data-url": "openSettingPage()",
						"icon": `${THEME.url}/assets/img/bookmarks/settings.png`
					},
					{
						"name": "扫一扫",
						"data-url": "openscanWebsite()",
						"icon": `${THEME.url}/assets/img/bookmarks/qrscan.png`
					}
				)
				if (!THEME.auth) {
					json.push(
						{
							name: "<font color=\"red\">本站同款系统</font>",
							url: 'http://guide.bri6.cn',
							icon: 'http://guide.bri6.cn/favicon.ico',
							target: '_blank'
						}
					)
				}
	...
	bookMarkFn.prototype = {
		init: function () {
			...
			for (var i = 0, l = data.length; i < l; i++) {
				if (data[i].icon) {
					html += `<img style="display: none;" src="${data[i].icon}" referrer="no-referrer">`
				}
				html += '<a class="list" data-url="' + (data[i]['data-url'] ? data[i]['data-url'] : '') + '" target="' + (data[i].target ? data[i].target : '') + '" rel="' + (data[i].rel ? data[i].rel : '') + '"' +
					(data[i].url ? 'href="' + data[i].url + '"' : '') + '><div class="img" style="background-image:url(' + data[i].icon + ')"></div><div class="text">' + data[i].name + "</div></a>";
				// html += '<a class="list" data-url="' + data[i].url + '"><div class="img" style="background-image:url(' + data[i].icon + ')"></div><div class="text">' + data[i].name + "</div></a>";
			}
			this.$ele.html(html);
			bookmarkNumSet();
			this.bind();
		...
		insertPage: function (siteData) {
			...
			for (let i in libData) {
				libHTML += `<div index="${i}"><img src="${libData[i].icon}"><p>${libData[i].name}</p></div>`;
			}
	...
	function openSettingPage() {
		...
		$(".set-markdown-page").html(
			`<iframe src="${THEME.url}/html/README.html?bg=${$(".page-settings").css("backgroundColor")}&color=${$(".page-settings").css("color")}"></iframe>`
		)
	...
	function changeREADMEtheme() {
		if ($(".set-markdown-page iframe")[0]) {
			var src =
				`${THEME.url}/html/README.html?bg=${$(".page-settings").css("backgroundColor")}&color=${$(".page-settings").css("color")}`;
